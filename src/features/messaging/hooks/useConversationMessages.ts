...
export const useConversationMessages = ({
    ...
    (
      ...
      const tryInsert = (
        ...
        if (lastContentTimestamp != null) {
          ...
          if (
            fromNext ? diff <= contentWindowNanos : diff < contentWindowNanos
          ) {
            return;
          }
        }
  ...
  const fetchPage = useCallback(
    async ({
      ...
      if (isGroupChat) {
        ...
        result = await fetchPaginatedDmThreadMessages(
          ...
          accessGroupsRef.current,
          {
            afterCursor: cursor,
            limit: MESSAGE_PAGE_SIZE,
            fallbackBeforeTimestampNanos: cursor
              ? paginationTimestamp
              : undefined,
          },
        );
      ...
      if (missingKeys.size > 0) {
        const fetchedProfiles = await fetchProfilesBatch(
  ...
  const {
    ...
    PageData,
    Error,
    InfiniteData<PageData, PageParam>,
    ReturnType<typeof messageKeys.thread>,
    PageParam
  >({
    queryKey: messageKeys.thread(conversationId),
    queryFn: ({ pageParam }) =>
      fetchPage((pageParam as PageParam) ?? initialPageParam),
      ...
      queryClient.setQueryData(
        ...
        (oldData: any) => {
          if (!oldData) {
            ...
            return {
              pageParams: [
                { cursor: null, beforeTimestamp: null, isInitial: true },
              ],
              pages: [
                {
  ...
  useEffect(() => {
    ...
    channel.on("broadcast", { event: "message" }, async ({ payload }) => {
      ...
      const encryptedMessage: NewMessageEntryResponse = {
        ChatType:
          (messagePayload.metadata?.chatType as ChatType) || ChatType.DM,
        SenderInfo: {
          OwnerPublicKeyBase58Check: messagePayload.senderPublicKey || "",
          AccessGroupPublicKeyBase58Check:
            messagePayload.SenderAccessGroupPublicKeyBase58Check || "",
          AccessGroupKeyName: messagePayload.SenderAccessGroupKeyName || "",
        },
        RecipientInfo: {
          OwnerPublicKeyBase58Check:
            (messagePayload.recipients && messagePayload.recipients[0]) || "",
          AccessGroupPublicKeyBase58Check:
            messagePayload.RecipientAccessGroupPublicKeyBase58Check || "",
          AccessGroupKeyName: messagePayload.RecipientAccessGroupKeyName || "",
        },
        MessageInfo: {
          EncryptedText: messagePayload.EncryptedMessageText || "",
          TimestampNanos: Number(
            messagePayload.timestampNanos || Date.now() * 1e6,
          ),
  ...
  }, [conversationId, mergeMessages, userPublicKey]);
