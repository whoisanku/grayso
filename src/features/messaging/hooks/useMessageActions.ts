...
type UseMessageActionsProps = {
    ...
    recipientInfo?: any;
    setMessages: React.Dispatch<React.SetStateAction<DecryptedMessageEntryResponse[]>>;
...
export const useMessageActions = ({
    ...
    const bubblePreviewStyle = useAnimatedStyle(() => ({
        opacity: actionSheetAnim.value,
        transform: [
            { translateY: interpolate(actionSheetAnim.value, [0, 1], [10, 0]) },
            { scale: interpolate(actionSheetAnim.value, [0, 1], [0.97, 1]) },
        ],
    }));
