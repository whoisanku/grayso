...
export const useConversationThreads = (
  ...
  >({
    ...
    queryFn: async ({ pageParam }) => {
      try {
        return await fetchConversationThreads(
    ...
    staleTime: 8000,
    refetchInterval: (options.isFocused ?? true) ? 8000 : false,
  ...
  useEffect(() => {
    ...
    console.log(
      "[useConversationThreads] Subscribing to global channel:",
      channelIdentifier,
    );
    ...
    const channel = supabase
      ...
      .on("broadcast", { event: "typing" }, ({ payload }: { payload: any }) => {
        ...
        const { conversationId, is_typing, senderPublicKey, metadata } =
          payload;
        ...
        const key =
          metadata?.chatType === "DM" && senderPublicKey
            ? senderPublicKey
            : conversationId;

        console.log(
          "[useConversationThreads] Derived typing key:",
          key,
          "is_typing:",
          is_typing,
        );
      ...
      .on(
        ...
        ({ payload }: { payload: any }) => {
          console.log(
            "✅ [useConversationThreads] Received new_message broadcast event:",
            payload,
          );
          if (!isMounted) return;
          console.log(
            "[useConversationThreads] Processing new_message event:",
            payload,
          );
          ...
          if (!message) {
            console.warn(
              "[useConversationThreads] Payload missing message object",
            );
            return;
          }
          ...
          const key =
            message?.ChatType === "DM" && senderPublicKey
              ? senderPublicKey
              : conversationId;
          ...
          if (key && message) {
            ...
            let recipientOwnerPk =
              message.RecipientInfo?.OwnerPublicKeyBase58Check ||
              message.RecipientAccessGroupOwnerPublicKeyBase58Check ||
              "";
            ...
            if (!recipientOwnerPk && userPublicKey) {
              const senderPk =
                message.SenderInfo?.OwnerPublicKeyBase58Check ||
                message.SenderAccessGroupOwnerPublicKeyBase58Check ||
                "";
              ...
              if (senderPk && senderPk !== userPublicKey) {
                ...
                const otherPart = parts.find(
                  (p: string) => p !== userPublicKey && p.startsWith("BC1"),
                );
                if (otherPart) {
            ...
            if (!senderAccessGroupPk && senderOwnerPk) {
              try {
                ...
                console.warn(
                  "[useConversationThreads] Failed to fetch sender access groups",
                  e,
                );
              }
            }

            const recipientAccessGroupPk =
            ...
            if (!userPublicKey) {
              console.warn(
                "[useConversationThreads] Missing user public key for decryption",
              );
              return;
            }
            ...
            void (async () => {
              try {
                ...
                if (!senderAccessGroupPk && senderOwnerPk) {
                  try {
                    const senderGroups = await getAllAccessGroups({
                      PublicKeyBase58Check: senderOwnerPk,
                    });
                    const allSenderGroups = (
                      senderGroups.AccessGroupsOwned || []
                    ).concat(senderGroups.AccessGroupsMember || []);
                    senderAccessGroupPk =
                      findAccessGroupPublicKey(
                        allSenderGroups,
                        senderOwnerPk,
                    ...
                    console.log(
                      "[useConversationThreads] Fetched sender access group key:",
                      senderAccessGroupPk,
                    );
                  } catch (e) {
                    console.warn(
                ...
                const messageToDecrypt = {
                  ...
                  ChatType:
                    message.ChatType ||
                    (message.RecipientAccessGroupKeyName ? "GROUPCHAT" : "DM"),
                  SenderInfo: {
                  ...
                  MessageInfo: {
                    ...
                    ExtraData:
                      message.MessageInfo?.ExtraData || message.ExtraData || {},
                  },
                };
                ...
                if (decrypted.length > 0) {
                  ...
                  if (decryptedMsg.error) {
                    console.warn(
                      "[useConversationThreads] Decryption returned error:",
                      decryptedMsg.error,
                    );
                  } else {
                ...
                if (!isMounted) return;
                console.warn(
                  "[useConversationThreads] Failed to decrypt new_message payload",
                  error,
                );
              }
            })();
      ...
      .subscribe((status, err) => {
        ...
        if (status === "SUBSCRIBED") {
          ...
          if (!hasLoggedErrorRef.current) {
            console.warn(
              `[useConversationThreads] Channel ${status}. Will attempt reconnects silently.`,
            );
    ...
    return () => {
      isMounted = false;
      console.log(
        "[useConversationThreads] Cleaning up global channel:",
        channelIdentifier,
      );
      channel.unsubscribe();
    };
