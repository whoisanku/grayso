...
const extractVideoId = (url: string): string | null => {
  try {
    ...
    return parsed.hostname === "iframe.videodelivery.net"
...
const normalizeVideoUploadMetadata = (
  ...
  const appendBaseMetadata = (clientIdValue: string) => {
    ...
    extraData[`video.${index}.orientation`] =
...
export const Composer = React.memo(function Composer({
  ...
  const uploadMediaItem = async (item: SelectedImage) => {
    ...
    try {
      ...
      const onProgress = (progress: number) => {
        setSelectedImages((prev) =>
          prev.map((img) => (img.uri === item.uri ? { ...img, progress } : img))
        );
      };
      ...
      setSelectedImages((prev) =>
        prev.map((img) =>
          img.uri === item.uri
            ? { ...img, uploadStatus: "error", progress: 0 }
            : img
        )
      );
    }
  ...
  const onSend = useCallback(
    async (messageText?: string) => {
      ...
      try {
        ...
        if (
          replyToMessage &&
          replyToMessage.MessageInfo?.TimestampNanosString
        ) {
          extraData.RepliedToMessageId =
            replyToMessage.MessageInfo.TimestampNanosString;
          if (replyToMessage.MessageInfo.EncryptedText) {
            extraData.RepliedToMessageEncryptedText =
              replyToMessage.MessageInfo.EncryptedText;
          }
        }
  ...
  const replyPreview = useMemo(() => {
    ...
    const displayName = senderPk
      ? getProfileDisplayName(senderProfile, senderPk)
      : "Unknown";
    const messageText =
      getDisplayedMessageText(replyToMessage) || "Message not loaded";
    ...
    if (isEditMode) {
      ...
      try {
        ...
        if (selectedImages.length > 0) {
          ...
          const pendingUploads = selectedImages.filter(
            (img) =>
          ...
          const failedUploads = selectedImages.filter(
          ...
          try {
            const imagesToUpload = selectedImages.filter(
              (img) => !img.mimeType?.startsWith("video/")
            );
            const videosToUpload = selectedImages.filter((img) =>
        ...
        if (optimisticVideoUrls.length > 0) {
          optimisticExtraData.decryptedVideoURLs =
            JSON.stringify(optimisticVideoUrls);
        }
            ...
            >
              {sending || isSavingEdit ? (
                ...
                <Ionicons
                  ...
                  size={22}
                  color={
                    !hasContent ? (isDark ? "#94a3b8" : "#64748b") : "#ffffff"
                  }
                />
              )}
            </View>
