...
export const MessageBubble = React.memo(function MessageBubble({
  ...
  const messageText =
    (isEditedMessage && editedMessageText
      ? editedMessageText
  ...
  const isMediaOnly =
    hasMedia && (!normalizedMessageText || normalizedMessageText.length === 0);
  ...
  const renderReplyPreview = () => {
    ...
    const fallbackText =
      item.MessageInfo?.ExtraData?.RepliedToMessageDecryptedText;
    const replyText =
    ...
    return (
      <View
        style={{
          backgroundColor: isMine ? "rgba(255, 255, 255, 0.15)" : accentSoft,
          borderLeftWidth: 3,
  ...
  const isPreviousMessageFromSameSender =
    previousMessage?.IsSender === item.IsSender;
  ...
  const isPreviousMessageClose =
    previousMessage && timestamp && previousMessage.MessageInfo?.TimestampNanos
      ? Math.abs(timestamp - previousMessage.MessageInfo.TimestampNanos) <
        MESSAGE_GROUPING_WINDOW_NS
      : false;

  const isFirstInGroup =
    !isPreviousMessageFromSameSender || !isPreviousMessageClose;
      ...
      >
        ...
        <View style={{ position: "relative" }}>
          ...
          >
            ...
            <View
              className={
                hasMedia && !isMediaOnly ? "flex-col gap-2" : undefined
              }
            >
  ...
  const messageRow = (
    <Reanimated.View style={animatedRowStyle}>{bubbleContent}</Reanimated.View>
  );

  return (
    <View style={{ marginBottom }} ref={bubbleContainerRef}>
      {showDayDivider ? (
      ...
      <View style={{ position: "relative" }}>
        ...
        {/* Swipe-to-reply is native-only to avoid touch interference on web/mobile PWA */}
        {enableSwipeToReply ? (
